...
一个 API-first、可配置、可执行、可测试、可评测的轻量 Text-to-SQL Engine。调用方通过 API 提交自然语言问题，系统基于可信上下文（Schema、Reference SQL、文档知识、Metric、Semantic Model）运行可配置多阶段工作流，生成、校验、只读执行并有限修复 SQL，全过程保留可观测 Trace。

> 当前优先保证核心调用链真实可运行、数据库查询安全可验证、节点输入输出清晰、错误可分类和有限修复，以及测试和评测可复现。当前不建设图形界面、认证、多租户、调度或 BI 看板。详见文末[项目定位与边界](#项目定位与边界)。
...
- **可观测 API**：每个节点执行后记录 Trace；API 返回 SQL、只读查询结果、修复历史、错误和 Trace，并提供运行记录、保存 SQL、反馈及运行时配置接口。
...
- 默认 LLM 走 OpenAI-compatible provider，需要可用的 API Key；测试和离线验证场景使用 Mock LLM，不依赖真实付费 API
...
如果只是验证成功、修复和终止路径，可以直接使用 `python scripts/run_demo.py`。该开发脚本显式注入 `MockLLMClient`，不需要 API Key，也不代表正式 API 的默认 Provider。
...
运行三条内置验证场景（使用 Mock LLM，无需 API Key）：
...
详细讲解见 [docs/面试演示场景.md](docs/面试演示场景.md)。该文档中的样例属于开发 Fixture，不是正式运行时默认数据。
...
- `POST /api/v1/runtime/configs`：创建临时运行配置（数据库连接 + `light/strong` 模型路由），返回的 `runtime_config_id` 可用于 `/api/v1/query`、`/api/v1/schema` 和 `/api/v1/sql/execute`；运行时配置只保存在后端内存中，用户提交的密码和 API Key 不会写入配置文件，也不会出现在 API 响应中
...
├── README.md                  # 项目介绍、启动方式、离线验证场景、API 示例
...
├── data/sqlite/               # 本地开发与测试 Fixture 数据库
├── docs/                      # 项目说明文档
├── examples/                  # 最小工作流配置示例
...
- **定位**：API-first、可配置、可执行、可测试、可评测的轻量 Text-to-SQL Engine。调用方通过 HTTP API 使用能力，当前不维护图形界面。
