...
_EXPECTED_FILES = ("sakila-schema.sql", "sakila-data.sql")
_REQUIRED_KEYS = {
    "source",
    "sakila_version",
    "archive_url",
...
def _validate_manifest(manifest: dict[str, Any]) -> None:
    if set(manifest) < _REQUIRED_KEYS:
        raise ValueError("Sakila manifest is invalid")
    ...
    for name in _EXPECTED_FILES:
        ...
        if (
            ...
            or entry.get("member") != f"sakila-db/{name}"
...
def load_manifest(path: Path) -> dict[str, Any]:
    try:
        ...
        raise ValueError("Sakila manifest is invalid") from error
...
def _existing_files(
    ...
    if (
        ...
        return SakilaFiles(schema=schema, data=data)
    return None
...
def extract_verified_archive(
    ...
    if _sha256_file(archive_path) != manifest["archive_sha256"]:
        raise ValueError("Sakila archive verification failed")
        ...
        with tarfile.open(archive_path, "r:gz") as archive:
            for name in _EXPECTED_FILES:
                ...
                try:
                    ...
                    raise ValueError(
                        "Sakila member verification failed"
                    ) from error
                stream = archive.extractfile(member)
                if stream is None:
                    raise ValueError("Sakila member verification failed")
                content = stream.read()
                if _sha256_bytes(content) != entry["sha256"]:
                    raise ValueError("Sakila member verification failed")
                contents[name] = content
    except (tarfile.TarError, OSError) as error:
        raise ValueError("Sakila archive verification failed") from error
...
def fetch_sakila(manifest_path: Path, target_dir: Path) -> SakilaFiles:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Fetch and verify the locked official Sakila fixture."
    )
    ...
    files = fetch_sakila(args.manifest, args.output)
    print(
        f"Verified Sakila fixture retrieved at "
        f"{manifest['retrieved_at']} from {manifest['source']} into "
        f"{files.schema.parent}"
    )
