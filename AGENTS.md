...
1. private_context/、个人简历、面试知识库和其他私有资料不得提交到 Git。
2. 不得把其中的手机号、邮箱、公司内部资料、未公开指标或项目描述复制到公开仓库。
3. 私有资料不得进入 README、docs、代码注释、示例配置、测试数据或日志。
4. 如果发现私有文件未被 .gitignore 忽略，先报告风险并给出修复方案。
5. 不得删除或修改私有资料，除非用户单独明确要求。
