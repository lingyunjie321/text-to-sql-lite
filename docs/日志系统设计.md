...
当前项目已经具备工作流级 `TraceEvent`，可以把单次 Text-to-SQL 运行过程返回给 API 和前端开发者调试面板。这个 Trace 适合展示链路和解释节点行为；同时，项目已经落地 demo 级结构化日志系统，覆盖统一 `logging` 初始化、请求日志、结构化运行事件、文件日志、日志轮转、异常源位置记录和敏感信息脱敏。
...
因此日志系统不能只围绕工作流节点做可观测性，还必须覆盖服务初始化、配置解析、LLM provider、数据库连接、异常分类和敏感信息保护。
...
7. 引入项目自定义异常体系，替换主链路中排障价值高的散乱 `ValueError`。
...
1. 当前不接入 ELK、Loki、Datadog、Sentry、OpenTelemetry Collector 等外部平台。
2. 当前不实现完整 metrics 系统，例如成功率、P95、节点耗时分布等聚合指标。
3. 当前不把日志写入数据库。
4. 当前不把完整 traceback 默认写入日志；只在 debug 或显式配置打开时输出。
...
统一定义项目异常体系，将主链路中的散乱 `ValueError` 收敛为明确异常类型。
...
- `prompt_preview=disabled` 表示当前不记录 prompt preview，只允许记录 prompt 长度、hash 和 summary。
...
1. 项目异常体系和脱敏工具。
2. 控制台文本日志、JSON Lines 文件日志和每日轮转 handler。
3. FastAPI 请求日志中间件。
4. 服务初始化、数据库 URL 解析、LLM client 构建日志。
5. 工作流开始/完成/失败、节点开始/完成/失败日志。
6. SQL 校验失败、SQL 执行失败、修复尝试和修复耗尽日志。
7. OpenAI-compatible provider 调用成功/失败日志。
8. 单元测试和集成测试覆盖格式化、脱敏、异常源定位、请求日志、工作流日志和关键失败路径。
...
10. `ruff check .` 和 `python -m pytest` 通过。
