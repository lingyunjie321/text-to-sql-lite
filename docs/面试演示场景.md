...
本文档用于面试时快速展示 Text-to-SQL Agent Demo 的主链路、修复闭环和终止条件。所有场景都使用 `MockLLMClient`，不需要 API Key，也不依赖真实在线 LLM。
...
- `GenerateSQLNode` 内的 `ComplexityClassifier` 识别聚合、Top-N/排名和多表关联，路由到 `strong` alias。
...
- `ReflectErrorNode` 生成结构化 `repair_instruction`。
- `FixSQLNode` 记录 `old_sql`、`new_sql`、`error_type` 和 `reason`。
