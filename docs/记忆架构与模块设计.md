...
- 成功 finalization 会写入 `reflection_strategy="SUCCESS"` 的最终 SQL 记忆。
- `query_run`、`trace_event`、`saved_query`、`feedback` 已作为 Run Memory 保存。
- `status="approved"` 的 saved_query 可作为可信 Reference SQL 进入后续 `rag_context.reference_sql`。

## 仍然缺什么

- 成功后智能反思 `ExecutionReflectionNode`：当前只有固定的 `SUCCESS` 记忆，没有对成功 SQL 做质量、口径或可复用性判断。
- feedback -> candidate -> approved knowledge 的完整治理流：当前 status 字段存在，但没有审核 API/UI。
...
| Phase 4 | 轻量知识审核 API/UI | 支持数据团队把 candidate 审核为 approved，或标记 deprecated。 |
