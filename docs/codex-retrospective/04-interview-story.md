...
我做过一个轻量级 Text-to-SQL Agent demo，重点不是让 LLM 直接拼 SQL，而是用 Codex/Claude Code 这类 AI 编程工具，把模糊需求拆成项目级工程流程：需求澄清、架构设计、自研 workflow、节点注册、Prompt 裁剪、SQLGlot 校验、最多 3 次反思修复、Trace、Mock LLM 测试和 React 前端演示。这个项目能展示我如何约束 AI 输出，而不是完全放任 AI 写代码。需要诚实说明的是：现在整理的 Prompt Playbook 是基于当前仓库反向复盘的重构版，不是当时逐字历史 Prompt。

## 2 分钟版本

这个项目的背景是：运营或分析师希望用自然语言查业务数据，但 Text-to-SQL 最大风险是 LLM 生成错误或危险 SQL。所以我把目标拆成一个轻量业务交付版 demo：数据团队维护 Schema、Reference SQL、文档知识、Metric 和 Semantic Model，后端通过可配置 workflow 生成、校验、执行和修复 SQL，前端展示 SQL、结果、Trace 和修复历史。
...
3. 脚手架 Prompt：先建立 FastAPI、pytest、ruff、Vite、Vitest。
...
每轮 Prompt 都有 Goal / Context / Constraints / Done when / Verification / Failure rule。比如我会明确要求：如果 `ruff check .`、`python -m pytest`、`npm test` 或 `npm run build` 失败，必须停止新增功能，先修复失败。
...
| 验证 | 把 lint/typecheck/test/build 写进 Prompt | 后端 `ruff check .`、`python -m pytest`；前端 `npm test/typecheck/build` |
...
- 前端支持运行配置、SQL 编辑执行、结果展示、历史、反馈和 Debug Trace。
...
前端让面试官能看到完整产品体验：自然语言输入、SQL 结果、运行配置、SQL 编辑执行、历史、反馈和 Debug Trace。它证明项目不是单脚本，而是有 API 契约和用户工作流。
...
- 建立可观测 Trace、结构化日志、metadata store、React/Vite 演示工作台和 pytest/Vitest 测试体系。
